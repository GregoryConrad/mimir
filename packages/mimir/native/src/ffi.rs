...
use crate::{
    embedded_milli,
    proto::{
        instance_ffi_request::{self, index_request},
        instance_ffi_response, InstanceFfiRequest, InstanceFfiResponse, TermsMatchingStrategy,
    },
};
...
#[no_mangle]
pub extern "C" fn embedded_milli_process_instance_request(buffer: DartBuffer) -> RustBuffer {
    // SAFETY: Dart side ensures buffer is a valid block of memory
    let buffer = unsafe { std::slice::from_raw_parts(buffer.ptr, buffer.len) };
